...
public class MainScreen {
    ...
    private JButton checkGameStartButton;
    ...
    public void buildMainScreen() {
        JPanel mainPanel = new MainPanel(dim);
        JFrame mainFrame = new MainFrame(dim, mainPanel);
        for (int r = 0; r < 9; r+=3) {
            var endRow = r + 2;
            for (int c = 0; c < 9; c+=3) {
                var endCol = c + 2;
    ...
    private void addFinishGameButton(final JPanel mainPanel) {
        finishGameButton = new FinishGameButton(e -> {
            if (boardService.gameIsFinished()) {
                showMessageDialog(null, "Parabéns, voce concluio o jogo!");
                resetButton.setEnabled(false);
                checkGameStartButton.setEnabled(false);
                finishGameButton.setEnabled(false);
            } else {
                var message = "Seu jogo tem alguma inconsistencia, ajuste";
    ...
    private void addCheckGameStatusButton(final JPanel mainPanel) {
        checkGameStartButton = new CheckGameStatusButton(e -> {
        ...
        mainPanel.add(checkGameStartButton);
    }
